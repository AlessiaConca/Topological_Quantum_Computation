...
\begin{definition} (Edge operator) 
	We define a generic operator $A$ acting on an edge $e$ of $\mathcal{L}$ as: $A_e$=$A_{(i+\frac{i'-i}{2}, j+\frac{j'-j}{2})}$ where the tuples $(i,j),(i',j')$ uniquely identify and edge $e\in E$.
\end{definition}
...
\begin{center}
	$ A_{\vec{v}} = \prod_{star(\vec{v})} Z_e $ 
	
	$ B_{\vec{p}} = \prod_{bdy(\vec{p})} X_e $.
\end{center}
