...
What follows naturally is that all the classes are $B_{\vec{p}} $-$invariant$ due to the topological characteristics of the torus. More formally we can write that given a class of eigenstates of $A_{\vec{v}} $ named $|\psi_1\rangle,...,|\psi_i\rangle,...,|\psi_n\rangle$ all belonging to the same class and forming state $|\psi\rangle$ and given that we know that $B_{\vec{p}} |\psi_i\rangle=+|\psi_j\rangle$, then if we apply $B_{\vec{p}} $ to the normalized state $|\psi\rangle$ we get:
