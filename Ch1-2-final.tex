...
\puttitle{
	...
	coadvisor={Dr. Massimo Moscolari}, % Co-Supervisor name, remove this line if there is none
...
\\
\\
\textbf{Keywords:} Quantum computing, Spin lattices, Anyons, Quantum error correction. % Keywords
...
\\
\\
\textbf{Parole chiave:} Computazione quantistica, Reticoli di spin, Correzione quantistica degli errori. % Keywords (italian)
...
If we go back to the configuration in figure 1.8, after the application of the operator $B_{\vec{p}} $ we do not land in class 1, but we remain in class 0; this is because we have an even number of horizontal intersections and an even number of vertical intersections, i.e. the indeces equal   $(\overline{0}, \overline{0})$.
...
\begin{definition}($S^x_\Sigma$ string operator)
	Given a string $\Sigma$, the string operator is given by $S_\Sigma^x = \prod_{j=1}^{n}\sigma^x_{e_j}$.
\end{definition}

\begin{definition}($S^z_{\Sigma'}$ string operator)
	Given a string $\Sigma'$, the string operator is given by $S^z_{\Sigma'} = \prod_{j=1}^{n}\sigma^z_{e_j'}$.
\end{definition}
...
In order to understand this, we study the relationships between string operators and vertex/plaquette operators as we have defined them (see section 1.2).\newline
%picture
\begin{figure}
...
\begin{center}
	...
	$S^x_\Sigma = \prod_{j=1}^{n} \sigma_{e_j}^x$
\end{center}
...
\begin{figure}
	\begin{center}
		\begin{tikzpicture}
			...
			\foreach \j in {1,...,1}
			{
				\draw[blue!50, line width=1.5mm] (0.5, \j) -- (1.5, \j);
				\draw[blue!50, line width=1.5mm] (-1.5, \j) -- (-0.5, \j);
				\node[draw, circle, fill=blue!50,label=center:\textbf{Z}] at (1.5,\j) {};
				\node[draw, circle, fill=blue!50,label=center:\textbf{Z}] at (0.5,\j) {};
				\node[draw, circle, fill=blue!50,label=center:\textbf{Z}] at (-0.5,\j) {};
				\node[draw, circle, fill=blue!50,label=center:\textbf{Z}] at (-1.5,\j) {};
...
\begin{thebibliography}{99} % Replace '99' with the widest label in your bibliography
	...
	\bibitem[Cha06]{Cha06} H.H. Chang, "An Introduction to Error-Correcting Codes: From Classical to Quantum", {\it arXiv: quant-ph/0602157}, 2006.
