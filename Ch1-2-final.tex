...
Among various quantum error correcting codes, topological codes are of particular interest due to their robustness against local perturbations and their ability to encode information in a non-local manner. The toric code, proposed by A. Kitaev, stands as a promising example of a topological code. \newline

The toric code was one of the first topological quantum error correcting codes that were proposed. It was introduced by Alexei Kitaev in 1997 as a two-dimensional lattice model. The toric code is particularly notable for its robustness against local errors thanks to its topological properties. 
In particular, the importance of Kitaev's work lies in its innovative solutions to address noise-mitigation. In fact, what makes the toric code special is the way it exploits the topological features of a quantum system to robustly encode information and protect it from the damaging effects of noise. In particular, not only does the toric code exhibit topological order, meanaing that its ground state is four fold degenerate, but it also exploits the existence of anyonic excitations of a two dimensional system to encode two qubits worth of information.
This topological approach offers a promising method to perform quantum error correction, showing great potential for creating more reliable and scalable quantum computers. 



%The toric code exhibits topological order and anyonic excitations. These two features make it a well-suited candidate for fault-tolerant quantum computation. \newline
