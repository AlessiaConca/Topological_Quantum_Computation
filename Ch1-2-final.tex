...
% ABSTRACT IN ENGLISH
\chapter*{Abstract} 
This thesis provides an detailed description of the toric code model, which plays a fundamental 
...
Questa tesi fornisce una descrizione dettagliata del modello del codice torico, che svolge un ruolo fondamentale nel campo della computazione quantistica come esempio paradigmatico di codice di correzione degli errori quantistici. Dopo una sezione introduttiva preliminare sugli osservabili di spin in Meccanica Quantistica, il primo capitolo della tesi contiene la descrizione del modello del codice torico e bdella sua Hamiltoniana seguito dal calcolo dei suoi stati fondamentali e stati eccitati, i quali vengono successivamente caratterizzati dando una panoramica delle statistiche di scambio fermioniche e bosoniche, con particolare attenzione alle applicazioni alla computazione quantistica del modello del codice torico. Dopo una breve panoramica sulla correzione degli errori classica e quantistica, l'analisi delle eccitazioni anyoniche del codice torico svolta nel primo capitolo viene utilizzata per descrivere esplicitamente la strategia dietro la correzione degli errori quantistici per qubit logici codificati negli stati fondamentali dell'Hamiltoniana del codice torico. 
\\
\\
...
quantum computation. 
Through the careful study of the theoretical features of the toric code model, this investigates some
...
which states that 	$\sigma^x_{\vec{v}}\sigma^z_{\vec{v}'}$ commutes for $\vec{v} \neq \vec{v}'$   but anticommutes for $\vec{v} = \vec{v}'$. This is follows from the anticommutation relationship of Pauli matrices $\sigma^x \sigma^z = - \sigma^x \sigma^z$. Thus, for an even numer of overlapping edges, in our case 2 or 4, the commutator becomes:\newline
...
The ground states of the toric code represent the system's lowest energy configuration. What makes these ground states particularly intriguing is its highly degenerate nature, meaning that there are numerous distinct configurations that all share the same minimum energy. 
The ground states exhibit topological order, characterized by non-local correlations between spins. This means that the model is robust  even when the system is subjected to local perturbations.  In fact, quantum entanglement between spins is not confined to neighboring pairs, instead, it extends over long distances and involves collective behaviours of spins that cannot be understood by considering them individually.
...
Each of these loops, having such properties, is an eigenstate of all $A_{\vec{v}} $ since, no matter how I locally apply $A_{\vec{v}} $, I will always preserve the sign of the state. 
...
Take a system of $N$ indistinguishable particles in three dimensions as in \cite{Cor23}, if we exchange any two of them such that $i<j$ we can only obtain a phase $\phi=\pm \pi$ in the resulting wave function:  
