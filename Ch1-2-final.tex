...
%In particular we discuss their self-adjointness, unitarity, commutation and anticommutation along with the definition of the spectrum of an observable, which will be foundamental concepts for the upcoming study. \newline
These properties are later to be used to give a precise description of the toric code model along with the peculiar properties of the operators involved in the definition of its Hamiltonian. Such operators, called vertex and plaquette operators, play a crucial role in the computation of the ground state of the system, menaing the state with the lowest possible energy in which the system can be.
In fact, because of their properties, vertex and plaquette operators are simultaneously diagonalizable and thusit is easy to compute the ground state of the Hamiltonian, which we discover is four fold degenerate 
Due to this the toric code exhibits topological order, which represents a key point in order to understand how to encode quantum information. After having computed the ground states of the Hamiltonian we study how excited states can be obtained, starting from them. Using strings of Pauli operators $\sigma^z$ and $\sigma^z$ we manage to raise the energy of the system by two units. In fcat, what we obtain are usual bosonic and fermionic excitations created in pairs. We focus on giving a brief characterization of bosonic and fermionic statistics of the excitations in order to expalin how mutual anyonic statistics arise in two dimensional systems such as the toric code and explain their braiding statistics. \newline
