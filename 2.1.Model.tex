...
The key part of the Toric Code are the so called "vertex" and "plaquette" operators that are going to be placed, respectively, on the vertices $v$ and cells $p$ of $\mathcal{L}$. Such operators can be defined formally as in definition 1.1 and implemented by means of Pauli matrices as exemplified in figure 1.2. \newline
...
\begin{center}
	...
	$ A_v \coloneq Z_{left} \otimes Z_{right} \otimes Z_{upper} \otimes Z_{bottom}$ 
	
	$ B_p \coloneq X_{left} \otimes X_{right} \otimes X_{upper} \otimes X_{bottom}$  

\end{center}
\end{definition}

where we have defined each $Z_i$ and $X_i$ to be equal to 
...
Each one of the $A_v$ and $B_p$ operators that appear in the Hamiltonian share some important properties, which are going to be covered in the following definitions and will later be used to further characterize the action of the operators. \newline

Firstly, notice that $A_v$ and $B_p$ operators are Hermitian and square to the identity. 
...
Given Proposition 1.1, we can know further characterize the opeartors in terms of their eigenvalues and define their spectrum. The spectrum of an operator in quantum mechanics represents the set of possible values that can be obtained through the measurements of that operator on a quantum state; some of them are also eigenstates of the operator. The trivial spectrum of vertices and plaquette operators will be of fundamental importance to perform fault-tolerant quantum compuations, in particular for measuring whether any error has occurred during computation (see Chapter 3). 

\begin{proposition} (Eigenvalues of $A_v$ and $B_p$ operators) Given Hermitian operators $A_v$ and $B_p$, which satisfy the involutory property, their associated eigenvalues are $\pm 1$.
\end{proposition}
...
\caption{Veretx and plaquette operators according to the coordinate system with origin in (0,0). Here the plaquette operator is implemented on $\mathcal{L}'$ in order to coherently define its edges from the point of view of $\mathcal{L}$ using vector $\vec{v}$.}
