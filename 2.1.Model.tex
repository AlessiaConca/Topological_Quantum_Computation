...
\begin{center}
	...
	$ Av \equiv Z_l \otimes Z_r \otimes Z_u \otimes Z_d$ \newline
	
	$ Bp \equiv X_l \otimes X_r \otimes X_u \otimes X_d$ \newline 

\end{center}
\end{definition}

For the sake of simplicity, the tensor product is omitted when there is no risk of ambiguity in the interpretation. As the vertex and plaquette operators act locally, here we can omit writing down the tensor product with the identities for sites that are not part of the support of the operator. Thus, we can simply rewrite the operators by means of a product on the quartets of spins by indexing them as $j \in star(v)$ and $j \in bdy(p)$. \newline
...
We now analyze the relationship between vertex and plaquette operators. Such relationship is characterized by two fundamental mathematical concepts called "commutation" and "anticommutation". These relations also serve as an important result in understanding error detection and fault-tolerance of the toric code (see Chapter 3).	
