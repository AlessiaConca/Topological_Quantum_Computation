...
Since all of these spins exhibit the same characteristics they are identified with identical particles, which will be useful to know to further study the properties of the model.
...
Note that, for the sake of simplicity, the tensor product can be omitted when there is no risk of ambiguity in the interpretation. Here, since the vertex and plaquette operators act locally, we can omit writing down the tensor product with the identity for sites that are not part of the support of the operator. Thus, we can simply rewrite the operators by means of the product of the Pauli matrices that act on the quartets of spins indexed as $j \in star(v)$ and $j \in bdy(p)$. 
...
\begin{proposition} (Eigenvalues of $Av$ and $Bp$ operators) Given Hermitian operators $Av$ and $Bp$, which satisfy the involutory property, their eigenvalues are $\pm 1$.
\end{proposition}
