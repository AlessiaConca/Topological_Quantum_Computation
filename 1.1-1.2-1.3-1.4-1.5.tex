...
Overall, we only have two classes of paths: those that do not involve any exchange as in class a and c, and those that are characterized by an exchange as in class b. \textit{If we let $\eta$ represent the phase acquired by the wave function under a single exchange, then since two exchanges are equivalent to no exchange $\eta^2=1$ and it follows that $\eta = \pm 1 $. \cite{Rao16}}. We can conclude that the only possible statistics in three dimensions are indeed the fermionic and bosonic ones.\newline
