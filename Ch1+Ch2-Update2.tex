...
such that $\lambda << 1 $, representing a reasonable perturbation with $j$ running over all the edges of the lattice. \cite{Kit02}. The ground states of the toric code Hamiltonian are protected from local perturbations of the form that we have described above.Though, we should be aware of the entity of the noise to which the system is subject to. In order to establish such entity, we focus on determining the two quantities given below, the second of which is strictly related with the concept of distance of a code. Given an initial state $ |k \rangle $ and  a different orthogonal state $ |\psi \rangle $ both part of the ground state of the Hamiltonian, if we want to know how much these states overlap, after having applied the perturbation $V$ for $n$ times, we compute the following terms:
