...
model in the fields of quantum error-correction and topological quantum computation. \newline
The study begins by going through the matehmatical properties of spin observables in quantum mechanics, such as self-adjointness, unitarity, commutation and anticommutation. \newline
The study moves on to the description of the model of the toric code Hamiltonian followed by the computation of its ground states and excited states, which are later characterized by giving an overview of fermionic and bosonic exchange statistics, with a particular focus on the creation of mutual anyonic excitations in two dimensional systems.\newline
The second part of the thesis focuses on practical applications, giving an overview of both classical and quantum error correction techniques. Consequently, it exploits the theoretical background of the first part and focuses on the potential realization of quantum error-correction 
within the toric code, going through the examination of strategies for error detection and correction also explaining their feasibility mathematically.\newline
Through this comprehensive analysis, the aim is to offer a valid motivation for
...
Questo elaborato fornisce una descrizione completa del modello del toic code, un modello fondamentale di codice quantistico nell'ambito della correzione degli errori quantistica e della computazione quantistica per codici topologici.\newline
Lo studio comincia passando in rassegna le proprietà matematiche degli osservabili di Spin in Meccanica Quantistica, come ad esempio l'autoaggiunzione, l'unitarietà, la commutazione e l'anticommutazione.\newline
In seguito, lo studio si focalizza sulla descrizione del modello del toric code con la sua Hamiltoniana. Segue la derivazione dei suoi stati fondamentali ed eccitati, i quali saranno in seguito caratterizzati dando una breve overview delle diverse statistiche di bosoni e fermioni, con particolare attenzione alla creazione di statistiche anioniche.\newline
La seconda parte dell'elaborato si concentra sulle applicazioni pratiche, dando una breve panoramica dei modelli di errore usati in ambito classico e quantistico per studiare la correzione degli errori di codici. In seguito, si fa uso degli strumenti teorici studiati nella prima parte per esporre la potenziale realizzazione della correzione quantistica di errori nel toric code, esaminando stretegie di rilevamento e  correzione di errori, spiegandone l'effettiva realizzabilità da un punto di vista matematico.\newline
Attraverso questa analisi approfondita, lo scopo è di offrire una valida motivazione allo studio di eccitazioni anioniche mediante peculiarità topologiche rilevanti proprie dei codici topologici come il toric code. 
\\
\\
...
Being able to leverage quantum phenomena for computation has brought significant 
advancements in the field of quantum computing.
Quantum systems possess unique properties promising exponential speedup over 
classical algorithms for specific tasks. However, realizing such potential 
needs reserachers to overcome significant challenges, such as the exposure to
noise of a physical system. 
As a consequence, quantum error correction has emerged as an important area of research. 
Indeed, several quantum computing models are being studied theoretically
in order to give access to their practical realizations. 
In particular, one calss of quantum codes called topological codes 
has raised interest in recent years. Such codes leverage topological features of 
physical systems to create quasiparticles that are able to perform error detection 
and correction techniques. 
The toric code, the focus of this thesis, exemplifies one of such systems. \newline


In the first part, we initially give a brief mathematical characterization
of Spin observables in Quantum Mechanics and study their mathematical properties.
These properties are later to be used to give a precise description of the toric code model 
along with the peculiar properties of its Hamiltonian. After the model description
we focus on computing the ground states of the Hamiltonian and study how excited states
can be obtained. Ultimately, we characterize the obtained excitations in terms of
quasiparticles by giving a brief overview of bosonic and fermionic statistics in 
order to explain how mutual anyonic statistics of quasiparticles arise in two 
dimensional systems. \newline
...
techniques in the way that they are typically performed both in the classical and quantum setting. 
Having
such methodologies in mind, we ultimately aim to build an analogy with QEC on the toric code.
In fact, the last section focuses solely on explaining why the anyonic excitations derived in the 
previous section are so important to be able to encode quantum states in the ground states 
of the Hamiltonian and why this is mathematically possible. Lastly, this section gives
a complete overview of possible error scenarios on the toric code in order to explain 
how error detection and correction are brought out in this context. \newline


In conclusion, this thesis aims to provide a comprehensive characterization of the 
...
such that $\lambda << 1 $, representing a reasonable perturbation with $j$ running over all the edges of the lattice. \cite{Kit02}. The ground states of the toric code Hamiltonian are protected from local perturbations of the form that we have described above but only if they are of moderate entity. In order to establish such entity, we focus on determining the two quantities given below, the second of which is strictly related to the concept of distance of a code. Given an initial state $ |k \rangle $ and  a different orthogonal state $ |\psi \rangle $ both part of the ground state of the Hamiltonian, if we want to know how much these states overlap, after having applied the perturbation $V$ for $n$ times, we compute the following terms:
...
\begin{figure}
	...
	\caption{Example of gate operators by means of braiding pairs of anyons on the toric code.}
...
In conclusion, this study has provided a comprehensive description of the toric 
code and it has examined its role in the field of quantum error correction. 
Throughout a detailed analysis of the spin observables in quantum mechanics and an
exhausive explanation of the model along with its properties we have gained 
insight into the functioning of toplogical codes.
Moreover, we have examined the practical implications of the toric code, starting
from error detection and correction both in the classical and quantum setting in 
the most general terms, all the way to the QEC explanation in the context of
the toric code model.\newline

As this study draws to an end, it becomes evident that while recent studies have 
made significant strides in understanding the toric code and its applications, 
there remain substantial opportunities for future development.
This, further motivates reserch to understand anyonic excitations and topological 
features of the toric code.\newline

Anyonic particles offer an interesting alternative to traditional fermions and 
bosons, with significant implications for the development of error-free 
codes. Additionally, the topological order and degeneracy of the ground state represent 
a key aspect of the system's robustness against local perturbations.
Research into anyonic excitations and topological order promises to open new 
frontiers into our understanding of quantum computing. \newline

In summary, the toric code, along with other similar models, is expected to become 
a fundamental part of quantum information. Therefore, future work should be expected to focus on advancing the theoretical understanding of the mathematical framework that provides the description of the TC model, together with the resulting practical implementations, in order to fully exploit its potential in realizing fault-tolerant 
quantum computation. \newline
