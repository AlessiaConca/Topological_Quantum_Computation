...
\begin{document}
  ...
  Recall that for a string operator, here we will choose $S^x$, both equations hold:
	...
	\begin{minipage}{1 \textwidth}
		...
		%In the Toric code, qubits are arranged on a two-dimensional lattice, typically in the shape of a torus. This lattice consists of rows and columns of qubits, forming a grid-like structure. Now, imagine drawing a loop that wraps around the torus in the horizontal direction and another loop that wraps around in the vertical direction, without crossing each other or intersecting themselves. These loops are what we call non-contractible loops because they cannot be continuously deformed or shrunk to a single point without leaving the surface of the torus.
		
		%These non-contractible loops play a crucial role in encoding quantum information in the Toric code. Specifically, the logical states of the two encoded qubits are determined by whether these loops are "traversed" an even or odd number of times. For example, if we define the logical states of the qubits as |0⟩ and |1⟩, we can associate one logical qubit with the loop along the horizontal direction and the other logical qubit with the loop along the vertical direction.
		...
		%Plaquette operators act on sets of qubits arranged in plaquette-shaped regions of the lattice. These operators are designed to detect errors that occur within these specific regions.
		...
		%Consequently, the measurement outcome of the plaquette operator may deviate from its expected eigenvalue of +1. This deviation indicates that an error has occurred within the plaquette region, as the observed measurement outcome is sensitive to the altered state of the qubits.\newline
		...
		%As a result, the measurement outcome of the vertex operator may deviate from its expected eigenvalue of +1. This deviation indicates that an error has occurred in the vicinity of the vertex, as the observed measurement outcome is sensitive to the altered correlations between neighboring qubits. \newline
