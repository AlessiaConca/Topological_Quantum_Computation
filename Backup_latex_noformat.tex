...
\begin{document}
  ...
  Recall that for a string operator, here we will choose $S^x$, both equations hold:
	...
	\begin{minipage}{1 \textwidth}
		...
		This means that errors cause the creation of anyons pairs that will move around the lattice. These pairs are connected through a single path. Then two things can happen: in the first case, anyons pairs self-annihilate by forming contractible closed loops, resulting in no detectable error; in the second case, if the loop is non-contactible what we obtain will be an implementation of a logical operation on the stored quantum state, of the types described above. These latter types of errors do not 'self-correct', therefore error correction will need to focus on this kind of errors. \newline
		...
		%In order to exemplify, take a $Z$ error on a single qubit. All the plaquette and vertex operators, which do not act on the qubit, commute with it. Therefore, the only remaining opearator that must anti-commute with it are the two veretx operators adjacent to it. This means that the error has caused the flipped the output of these latter operators. 
		...
		Once errors are detected through the measurement of vertex and plaquette operators, error-correction gates are applied to the qubits to rectify the errors. These gates typically involve the manipulation of anyons, which will be called 're-annihilation'. Though, if we recall the error model described in the previous section, where bit-flips and phase-flips errors happen independently with same probability $p$; we have to take notice that error-correction through re-annihilation of errors is possible only when $p$ is low, this is because in this way errors (anyons) tend not to propagate far away from where they originated and can be detected and rectified in an easier way. On the other hand, when $p$ increases, ambiguity arises as to how identify anyon pairs and error-correction may not succeed.
	     \newline
